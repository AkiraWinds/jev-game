...
| [`experiment/red-herring`](https://github.com/AkiraWinds/jev-game/commit/d3ea099bb20de0c43ab5117506834f9f17f1523c) | `d3ea099` | v2, 3-judge (`JUDGE_SLM_MODEL=gpt-4.1-nano`) | 100% | 381 ms | 60% | 659 ms | 90% | 725 ms |
| [`experiment/red-herring`](https://github.com/AkiraWinds/jev-game/commit/de9f89fc1203deeac0a4ececd59ae7d27501fc13) | `de9f89f` | **v2, 3-judge, rerun** (`JUDGE_SLM_MODEL=gpt-5.4-nano-2026-03-17`, `JUDGE_LLM_MODEL=gpt-5.4-2026-03-05`) | **100%** | **403 ms** | **100%** | **700 ms** | **100%** | **857 ms** |
...
- Adding a **third, small-model judge (SLM)** turned out to be the more
  reliable way to expose an accuracy gap — but the size of that gap tracked
  *which* small model was used far more than case design. The older/weaker
  `gpt-4.1-nano` SLM struggled on every variant it was tried on (60% on v2),
  while the newer `gpt-5.4-nano` SLM matched or nearly matched the large
  model everywhere except v1 baseline and v4 (90% on both). Model
  generation, not case difficulty, was the dominant factor in whether the
  SLM's accuracy gap showed up at all.
...
- All numbers are single 10-round samples; several rows (SLM's 90% on v1
  baseline and v4, 60% on the first v2 3-judge run) are single missed
  rounds worth re-running with more rounds/seeds before treating any gap as
  reliable.
